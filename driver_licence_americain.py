...
def rletter(initial):
    return initial.upper() if initial.isalpha() else "X"
...
# FORM
# -------------------------
st.title("Générateur Permis Californie")
...
office = st.selectbox("Field Office", list(offices.keys()))
...
if generate:
    ...
    dl = rletter(ln[0]) + rdigits(r,7)

    exp = datetime.date(iss.year+5, dob.month, dob.day)

    office_code = offices[office]
    seq = seq_random(r)

    dd = f"{iss.strftime('%m/%d/%Y')}{office_code}{seq}FD/{iss.strftime('%y')}"
